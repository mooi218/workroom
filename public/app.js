...
import { LANGUAGES, setLocale, getLocale, t, roleName } from "./i18n.js";
...
const settings = {
  ...
  locale: prefs.locale || "ja",
  theme: ["system", "dark", "light"].includes(prefs.theme)
    ? prefs.theme
    : "system",
...
function settingsUI() {
  ...
  const dark =
    settings.theme === "dark" ||
    (settings.theme === "system" && darkSystem.matches);
  document.documentElement.dataset.theme = dark ? "dark" : "light";
  $("#theme-toggle").textContent = dark ? "☀" : "☾";
  office.setTheme(dark);
}
...
function localize() {
  ...
  text(".credit-note>span:nth-child(2)", "creditLabel");
  attr(".tiny-info", "title", "creditHelp");
...
function applyState(next) {
  ...
  if (
    JSON.stringify([...picker.options].slice(1).map((o) => o.value)) !==
    JSON.stringify(values)
  ) {
    picker.replaceChildren(
      new Option(t("allProjects"), ""),
      ...values.map((p) => new Option(p, p)),
    );
    picker.value = project;
  } else if (picker.options[0]) picker.options[0].text = t("allProjects");
...
function render() {
  ...
  $("#floor-label").textContent = project || t("allProjects");
  for (const status of ["working", "waiting", "done"])
    $(`#${status}-count`).textContent = tasks.filter(
      (task) =>
        (!project || task.project === project) && task.status === status,
    ).length;
...
function renderDetails() {
  ...
  meta.textContent = task.project || t("projectUnset");
...
function renderHealth() {
  ...
  $("#demo-toggle").textContent = `${t(demo ? "liveButton" : "demoButton")} ↗`;
  const notice = $("#notice");
  if (demo) {
    notice.hidden = false;
    notice.textContent = t("demoNotice");
  } else if (
    !connected ||
